...
extern int               diskquota_max_database;
...
disk_quota_shmem_startup(void)
{
	...
	monitored_dbid_cache = DiskquotaShmemInitHash("table oid cache which shoud tracking", diskquota_max_database,
	                                              diskquota_max_database, &hash_ctl, HASH_ELEM, DISKQUOTA_OID_HASH);
	init_launcher_shmem();
...
{
	Size size;
	size = hash_estimate_size(MAX_NUM_TABLE_SIZE_ENTRIES / diskquota_max_database + 100, sizeof(TableSizeEntry));
...
DiskQuotaShmemSize(void)
{
	...
	size = add_size(size, hash_estimate_size(diskquota_max_database,
	...
	if (IS_QUERY_DISPATCHER())
	{
		...
		size = add_size(size, diskquota_worker_shmem_size() * diskquota_max_database);
	}
