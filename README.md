...
schemas or roles into the rejectlist in shared memory. Schemas or roles in 
rejectlist are used to do query enforcement to cancel queries which plan to 
...
reject list and another one is to save active table list. The reject list shared
...
If reject list shared memory is full, it's possible to load data into some 
