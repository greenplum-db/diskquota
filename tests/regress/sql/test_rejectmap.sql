--
-- This file contains tests for dispatching and quering rejectmap.
--

CREATE SCHEMA s_rejectmap;
SET search_path TO s_rejectmap;
    ...
    CASE block_type
      ...
      WHEN 'ROLE_TABLESPACE' THEN
        ...
        SELECT relowner INTO targetoid
          FROM pg_class WHERE relname=rel::text;
	END CASE;
    PERFORM diskquota.refresh_rejectmap(
    ARRAY[
      ROW(targetoid,
...
--
-- 1. Create an ordinary table and add its oid to rejectmap on seg0.
...
-- Insert an entry for blocked_t2 to rejectmap on seg0.
...
-- Insert an entry for blocked_t3 to rejectmap on seg0.
...
-- Insert an entry for blocked_t4 to rejectmap on seg0.
...
-- Insert an entry for blocked_t5 to rejectmap on seg0.
